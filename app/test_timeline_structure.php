...
try {
    ...
    echo "Structure de la table soins :\n";
    ...
    // Test 2: Créer un nouveau soin (sans remarques si la colonne n'existe pas)
    echo "2. Test de création d'un nouveau soin...\n";
    
    // Vérifier si la colonne remarques existe
    $query = "SHOW COLUMNS FROM soins LIKE 'remarques'";
    $stmt = $conn->prepare($query);
    $stmt->execute();
    $remarques_exists = $stmt->rowCount() > 0;
    ...
    if($remarques_exists) {
        ...
        $query = "INSERT INTO soins (patient_id, date_soin, heure_soin, type_soin, statut, created_at) 
                  VALUES (:patient_id, :date_soin, :heure_soin, :type_soin, :statut, NOW())";
    }
    ...
    if($stmt->execute()) {
        ...
        if($remarques_exists) {
            ...
            echo "ℹ️ Sans remarques (colonne non présente)\n";
        }
        echo "\n";
    } else {
    ...
    while($soin = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if($remarques_exists && !empty($soin['remarques'])) {
